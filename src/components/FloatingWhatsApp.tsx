...
export const FloatingWhatsApp: React.FC = () => {
  const whatsappUrl = `${siteConfig.whatsappUrl}?text=${encodeURIComponent(
    'Hello Solace Family Dental Care, I would like to inquire about an appointment.'
  )}`;
  ...
  return (
    <div className="fixed bottom-20 right-4 md:bottom-8 md:right-8 z-50 flex items-center group">
      ...
      <a
        ...
        aria-label="Chat with Solace Dental on WhatsApp"
        className="relative flex items-center justify-center w-14 h-14 rounded-full bg-[#25D366] text-[#FFFFFF] shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-[#25D366]/40"
      >
        {/* Pulse effect badge */}
        <span className="absolute top-0 right-0 w-3.5 h-3.5 bg-[#FFFFFF] border-2 border-[#25D366] rounded-full animate-ping" />
        <span className="absolute top-0 right-0 w-3.5 h-3.5 bg-[#25D366] border-2 border-[#FFFFFF] rounded-full" />
